...
const radios = document.querySelectorAll('input[type="radio"][name="rol"]');
const textInput_firma = document.querySelector('#firma');
const span_firma = document.querySelector('#span_firma')

radios.forEach(radio => {
  radio.addEventListener('change', () => {
    console.log(typeof radio.value)
    if (radio.value === '2' || radio.value === '3' || radio.value == 4) {
      textInput_firma.style.display = 'block';
      span_firma.style.display = 'block';
    } else {
      textInput_firma.style.display = 'none';
      span_firma.style.display = 'none';
    }
  });
});

// // Capturamos el campo de opciones múltiples y la tabla
// const opcionesSelect = document.getElementById('usuarios');
// // const opcionesSelect = document.querySelectorAll('option[name="usuarios"]')
// const tablaBody = document.getElementById('body-table');

// // Escuchamos el evento de cambio en el campo de opciones
// opcionesSelect.addEventListener('change', () => {
//   // Obtenemos las opciones seleccionadas
//   const opcionesSeleccionadas = Array.from(opcionesSelect.selectedOptions).map(option => option.value);

//   // Limpiamos la tabla
//   tablaBody.innerHTML = '';

//   // Iteramos sobre las opciones seleccionadas y agregamos filas a la tabla
//   opcionesSeleccionadas.forEach(opcion => {
//     const fila = document.createElement('tr');
//     const celda = document.createElement('td');
//     celda.textContent = opcion;
//     fila.appendChild(celda);
//     tablaBody.appendChild(fila);
//   });
// });

// document.querySelector('#select_usuario').addEventListener('change', function() {
//     var opcionSeleccionada = this.value;
//     console.log(opcionSeleccionada);
//     var tabla = document.querySelector('#tabla_usuarios_entidad');
//     var nuevaFila = tabla.insertRow(-1);
//     var nuevaCelda = nuevaFila.insertCell(0);
//     nuevaCelda.textContent = opcionSeleccionada;
// });

// Agregar un controlador de eventos para el cambio en el campo desplegable
selectUsuario.addEventListener('change', function() {
    // Obtener el valor y el texto del elemento seleccionado
    const value = selectUsuario.value;
    const label = selectUsuario.options[selectUsuario.selectedIndex].text;

    // Hacer una solicitud AJAX para obtener el rol del usuario
    fetch(`/get_usuario_rol/${value}/`)
